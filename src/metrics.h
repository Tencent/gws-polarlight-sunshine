	...
	using metrics_callback_func =  std::function<unsigned long long ()>; 
    ...
    public:
		...
		void register_metrics (std::string metrics_key,
			...
			if (setId >= 0 && counterId >= 0) {
				counter_map.emplace(std::make_pair(CombineIds(setId, counterId), metrics_callback_info{callback_func, callback_func_reset}));
			}
			unlock_metrics();
		};
		unsigned long long query_metrics (std::string metrics_key) {
		...
		unsigned long long query_metrics (int setId,int counterId) {
			metrics_callback_info my_func;
			bool func_found = false;
			lock_metrics();
