...
namespace video
{
  extern void set_display_changed(std::vector<std::string>& output_list);
}
#ifdef _WIN32
LRESULT CALLBACK
    ...
    case WM_DISPLAYCHANGE: {
      ...
      while (EnumDisplayDevices(nullptr, displayIndex, &displayDevice, 0)) {
        if(displayDevice.StateFlags & DISPLAY_DEVICE_ACTIVE) {
          ...
          std::cout <<"EnumDisplayDevices:"<< displayDevice.DeviceName <<","<< displayDevice.StateFlags <<std::endl;
        }
        displayIndex++;
      }
...
main(int argc, char *argv[]) {
  ...
  std::thread window_thread([]() {
    ...
    HDEVNOTIFY hDevNotify = RegisterDeviceNotification(wnd, &dbi, DEVICE_NOTIFY_WINDOW_HANDLE);
  ...
  if (AuroraMgr().Start() == true)
  {
      BOOST_LOG(info) << "AuroraMgr().Start() ture"sv;
  }
  else
  {
      BOOST_LOG(info) << "AuroraMgr().Start() false"sv;
  }
  AuroraMgr().SetLogWriteFunPtr((void*)log_write);
  // enable privacy mode 
  if(config::sunshine.privacy_mode) {
    /*
      std::string display_outputs;
      int physics = AuroraMgr().EnumDisplayOutputs(display_outputs);
      if (config::video.virtualDisplayCount > 1)
      {
    
          config::video.haveSecondOutput = true;
          BOOST_LOG(info) << "physics DisplayCount = " << physics;
          BOOST_LOG(info) << "main config::video.output_name = " << config::video.output_name;
          BOOST_LOG(info) << "main config::video.output2_name = " << config::video.output2_name;
          if (physics > 1)
          {
              if (config::video.output2_name.empty())
              {
                  config::video.output2_name = "\\\\.\\DISPLAY2";
              }
          }
      }
      
      if (config::video.virtualDisplayCount == 1 && physics == 1)
      {
          config::video.output2_name.clear();
          config::video.haveSecondOutput = false;
      }
      */
    std::size_t pos = config::sunshine.config_file.find("sunshine.conf");
    if (pos) {
      physical_display_data_file = config::sunshine.config_file.substr(0, pos);
      physical_display_data_file += "physical_displays.dat";
    }
    else {
      BOOST_LOG(error) << "sunshine.conf is missing. It is needed during privacy mode."sv;
    }

    /*
    //std::ifstream f(physical_display_config_file.c_str());  //if ofstream is used, file will always be created
    std::ifstream f(physical_display_data_file.c_str());  //if ofstream is used, file will always be created
    if (f.good()) { // file exists
        f.close();
    } 
    else { // file not exists
        std::ofstream out_f (physical_display_data_file.c_str());
        bool isConfigSaved = AuroraMgr().saveDisplayConfig((char *)physical_display_data_file.c_str());
        if (!isConfigSaved) {
          BOOST_LOG(info) <<  "can not save display data to file which is required in privacy mode"sv ;    
        }
    } // file not exists
    */
    std::ofstream out_f (physical_display_data_file.c_str());
   ...
   /*
    gNumPhysicalDisplays = AuroraMgr().EnumDisplayNames(gPhysicalDisplayNames);
    int numPhysical = gPhysicalDisplayNames.size();
    BOOST_LOG(info) <<  "********* sunshine startup, num of physical displays = "sv << gNumPhysicalDisplays ;   
    for (int i = 0; i < numPhysical; i++)
      BOOST_LOG(info) <<  "********* sunshine startup, physical display "sv << i << ":" << gPhysicalDisplayNames[i] ;   
   */
  } //privacy_mode
