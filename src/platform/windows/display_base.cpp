...
namespace platf::dxgi {
  ...
  display_base_t::init(const ::video::config_t &config, const std::string &display_name) {
    ...
    BOOST_LOG(info) << "display_base_t::init(), display_name = " << display_name;
    ...
    for (int x = 0; factory->EnumAdapters1(x, &adapter_p) != DXGI_ERROR_NOT_FOUND; ++x) {
      ...
      for (int y = 0; adapter_tmp->EnumOutputs(y, &output_p) != DXGI_ERROR_NOT_FOUND; ++y) {
        ...
        BOOST_LOG(info) << "display_base_t::init(), on EnumOutputs y = " << y 
                        <<", desc.DeviceName = "<< desc.DeviceName << ", output_name = " << output_name;
        if (!output_name.empty() && desc.DeviceName != output_name) {
          continue;
        }

        BOOST_LOG(info) << "display_base_t::init(), output_name is found, desc.AttachedToDesktop = " << desc.AttachedToDesktop;
        if (desc.AttachedToDesktop && test_dxgi_duplication(adapter_tmp, output_tmp)) 
        {
          output = std::move(output_tmp);
          BOOST_LOG(info) << "display_base_t::init(), AttachedToDesktop = true, test_dxgi_duplication = true, output is located.";
