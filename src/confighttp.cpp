...
namespace confighttp {
  ...
  getMetrics(resp_https_t response, req_https_t request) {
    ...
    for (auto &metrics_item : metrics_list) {
		...
		content += ":";
        auto val = MetricsMgr().query_metrics(metrics_item);
        std::snprintf(buf,100, "%ld",val);
        content += buf;			
		content += "\n";
	}
    response->write(content);
  ...
  getConfig(resp_https_t response, req_https_t request) {
    ...
    AuroraMgr().EnumDisplayOutputs(display_outputs);
  ...
  saveConfig(resp_https_t response, req_https_t request) {
    ...
    try {
      ...
      int v_disp = AuroraMgr().GetVirtualDisplayNum();
 ...
 void 
 virtualDisplay(resp_https_t response, req_https_t request) {
    if (!authenticate(response, request)) return;
