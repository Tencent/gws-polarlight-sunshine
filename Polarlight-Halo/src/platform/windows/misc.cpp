...
// MinGW header qos2.h define another type ULONG will build break
...
namespace platf {
  ...
  appPath() {
    WCHAR sunshine_path[MAX_PATH];
    GetModuleFileNameW(NULL, sunshine_path, _countof(sunshine_path));
    return std::filesystem::path { sunshine_path }.remove_filename();
  }
    ...
    if (!is_token_same_user_as_process(shell_token)) {
      ...
      if (!ret) {
        ...
        if (error == 740) {
          ...
          std::wstring elevated_command = L"elevator.exe ";
