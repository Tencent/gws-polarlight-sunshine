...
#define DEFAULT_APP_IMAGE_PATH HALO_ASSETS_DIR "/box.png"
...
namespace proc {
  ...
  validate_app_image_path(std::string app_image_path) {
    ...
    auto full_image_path = std::filesystem::path(HALO_ASSETS_DIR) / app_image_path;
    if (std::filesystem::exists(full_image_path)) {
      return full_image_path.string();
    }
    else if (app_image_path == "./assets/steam.png") {
      // handle old default steam image definition
      return HALO_ASSETS_DIR "/steam.png";
    }
    ...
    try {
      ...
      for (auto &[_, app_node] : apps_node) {
        ...
        if (!exclude_global_prep.value_or(false)) {
          prep_cmds.reserve(config::halo.prep_cmds.size());
          for (auto &prep_cmd : config::halo.prep_cmds) {
