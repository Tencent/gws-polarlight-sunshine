...
namespace upnp {
  ...
  start() {
    if (!config::halo.flags[config::flag::UPNP]) {
      return nullptr;
    }
    ...
    auto video2 = std::to_string(map_port(stream::VIDEO_STREAM_PORT2));
    ...
    std::vector<mapping_t> mappings {
      ...
      { video2, video2, "Video stream port"s, false },
