...
namespace rtsp_stream {
  ...
  public:
    rtsp_server_t(){
      MetricsMgr().register_metrics("Slots", 0, 5, [&]() {
      return (unsigned long long) (_slot_count);
    }, 
    [](){
    }
    );
  ...
  cmd_announce(rtsp_server_t *server, tcp::socket &sock, msg_t &&req) {
    ...
    auto session = stream::session::alloc(config,
      launch_session->gcm_key, launch_session->iv,
  ...
  rtpThread() {
    ...
    while (!shutdown_event->peek()) {
      ...
      //server.iterate(std::min(50000ms, config::stream.ping_timeout));
