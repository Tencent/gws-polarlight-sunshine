...
namespace config {
  ...
  namespace nv {
    ...
    typedef enum _NV_ENC_PARAMS_RC_MODE
    {
        ...
        NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ = 0x8, /**< Deprecated, use NV_ENC_PARAMS_RC_CBR + 
          NV_ENC_TWO_PASS_QUARTER_RESOLUTION / NV_ENC_TWO_PASS_FULL_RESOLUTION + lowDelayKeyFrameScale=1 */
        NV_ENC_PARAMS_RC_CBR_HQ = 0x10, /**< Deprecated, use NV_ENC_PARAMS_RC_CBR +
          NV_ENC_TWO_PASS_QUARTER_RESOLUTION / NV_ENC_TWO_PASS_FULL_RESOLUTION */
...
#undef _CONVERT_
  ...
  video_t video {
    ...
    false,  //afterhaveSecondOutput;
    false,  //bChangeSecondOutputFlag;
    //false,  //bOutPutSizeChange;
    0,
    true  // dwmflush
  };
  ...
  halo_t halo {
    ...
    {},  // Password
    {},  // Password Salt
    platf::appdata().string() + "\\halo.conf",  // config file
    {},  // cmd args
    47989,
    platf::appPath().string() + "log\\halo.log",  // log file
    {},  // prep commands
  };
  ...
  parse_option(std::string_view::const_iterator begin, std::string_view::const_iterator end) {
    ...
    endc = std::find_if(std::make_reverse_iterator(endc),
      std::make_reverse_iterator(begin),
      std::not_fn(whitespace)).base();
  ...
  map_int_int_f(std::unordered_map<std::string, std::string> &vars, const std::string &name, std::unordered_map<int, int> &input) {
    ...
    if (list.size() % 2) {
      std::cout << "Warning: expected "sv << name 
                << " to have a multiple of two elements --> not "sv << list.size()
                << std::endl;
      return;
    }
  ...
  apply_flags(const char *line) {
    ...
    while (*line != '\0') {
      switch (*line) {
        case '0':
          config::halo.flags[config::flag::PIN_STDIN].flip();
          break;
        case '1':
          config::halo.flags[config::flag::FRESH_STATE].flip();
          break;
        case '2':
          config::halo.flags[config::flag::FORCE_VIDEO_HEADER_REPLACE].flip();
          break;
        case 'p':
          config::halo.flags[config::flag::UPNP].flip();
          break;
        default:
  ...
  apply_config(std::unordered_map<std::string, std::string> &&vars) {
    if (!fs::exists(stream.file_apps.c_str())) {
      fs::copy_file(HALO_ASSETS_DIR "/apps.json", stream.file_apps);
    }
    ...
    string_f(vars, "output_fullname", video.output_fullname);
    string_f(vars, "output2_fullname", video.output2_fullname);
    string_f(vars, "output_filter", video.output_filter);
    int_f(vars, "virtualDisplayCount", video.virtualDisplayCount);
    ...
    path_f(vars, "log_path", config::halo.log_file);
    path_f(vars, "file_state", nvhttp.file_state);

    // Must be run after "file_state"
    config::halo.credentials_file = config::nvhttp.file_state;
    path_f(vars, "credentials_file", config::halo.credentials_file);

    string_f(vars, "external_ip", nvhttp.external_ip);
    list_string_f(vars, "resolutions"s, nvhttp.resolutions);
    list_int_f(vars, "fps"s, nvhttp.fps);
    list_prep_cmd_f(vars, "global_prep_cmd", config::halo.prep_cmds);
    ...
    int privacy_mode = 0;
    int_f(vars, "privacy_mode"s, privacy_mode);

    if (privacy_mode == 1) {
       halo.privacy_mode = true;
    }
  ...
  int 
  archive_log_file(const char *filepath) {
    ...
    if (stat(filepath, &attr) != 0) {
      std::cout 
        << "File " 
        << filepath 
        << " does not exist (if it's your first time to run sunshine, please ignore this message)" 
        << std::endl;
      return 0;  //it's OK if there is no previous log file, means first time to run sunshine
    } 

    if (attr.st_mtime == 0) {
      std::cout << "Can't get timestamp from File " << filepath <<std::endl;
      return -1;
    }

    // Get last modified time
    char timestr[100];
    strftime(timestr, sizeof(timestr), "%Y%m%d%H%M%S", localtime(&attr.st_mtime));
    // std::cout << "timestr: " << timestr << std::endl;

    std::string logFileName = filepath;
    std::string delimiter = ".log";
    std::string nameNoExt; 
    size_t pos = logFileName.find(delimiter);
    if (pos != std::string::npos) {
        nameNoExt = logFileName.substr(0, pos);
    }
    else{
        nameNoExt = logFileName;
    }

    // Construct new file name
    char newpath[100];
    strcpy(newpath, nameNoExt.c_str());
    strcat(newpath, ".");
    strcat(newpath, timestr);
    strcat(newpath, ".log");

    // Rename file  -- failed due to no permission
    if (rename(filepath, newpath) != 0) {
      std::cout << "Failed to rename File " << filepath << std::endl;
      //perror ( "Could not rename file" );
      return -1;
    }
    //else 
    //  std::cout << "File renamed to: " << newpath << std::endl;
    return 0;
  }
  ...
  delete_unnecessary_log_files(const char *path) {
    ...
    //DIR* dir = opendir(platf::appdata().string().c_str());
    //std::string dir_str = platf::appdata().string();
    std::string dir_str = platf::appPath().string() + "/log";
    ...
    while ((entry = readdir(dir)) != NULL) {     
      ...
      //if (name.substr(0, strlen("sunshine.log")) == "sunshine.log") {
      std::string delimiter = ".log";
    ...
    if (log_size > MAX_LOG_RETAINED) {
      for (int i = 0; i < log_size - MAX_LOG_RETAINED; i++) {
        if (remove(logs[i].name.c_str())!= 0) {
        //std::cout << "***************** failed to delete file - " << logs[i].name.c_str() << std::endl;
  ...
  parse(int argc, char *argv[]) {
    ...
    for (auto x = 1; x < argc; ++x) {
      ...
      if (line == "--help"sv) {
        ...
        if (*(line + 1) == '-') {
          halo.cmd.name = line + 2;
          halo.cmd.argc = argc - x - 1;
          halo.cmd.argv = argv + x + 1;
    ...
    /*if (!fs::exists(halo.config_file)) {
      std::ofstream { halo.config_file };  // create empty config file
    }
    else {
      archive_log_file(config::halo.log_file.c_str());
    } */
    
    delete_unnecessary_log_files (config::halo.log_file.c_str());

    auto vars = parse_config(read_file(halo.config_file.c_str()));
