...
namespace http {
  ...
  init() {
    ...
    if (user_creds_exist(config::halo.credentials_file)) {
      if (reload_user_creds(config::halo.credentials_file)) return -1;
    }
    else {
  ...
  reload_user_creds(const std::string &file) {
    ...
    try {
      pt::read_json(file, inputTree);
      config::halo.username = inputTree.get<std::string>("username");
      config::halo.password = inputTree.get<std::string>("password");
      config::halo.salt = inputTree.get<std::string>("salt");
    }
    catch (std::exception &e) {
