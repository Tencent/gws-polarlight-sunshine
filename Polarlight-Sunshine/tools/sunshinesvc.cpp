...
#define SVC_SUNSHINE_TERMINATED    ((DWORD)50003)
#define SVC_SERVICE_STOPED         ((DWORD)50004)
void 
...
RunTerminationHelper(HANDLE console_token, DWORD pid) {
  ...
  // send an event with process ID
  char process_cstr[100];
  snprintf(process_cstr, 100, "processID:%lu, exitCode: %lu", process_info.dwProcessId, exit_code);
  WriteReportEvent(SVC_SERVICE_STOPED, process_cstr);
    ...
    switch (WaitForMultipleObjects(_countof(wait_objects), wait_objects, FALSE, INFINITE)) {
      ...
      case WAIT_OBJECT_0 + 1:
        //Sunshine  terminated itself.
        {
          ...
          char process_cstr[100];
          snprintf(process_cstr, 100, "processID:%lu, exitCode: %lu", process_info.dwProcessId, exit_code);
          WriteReportEvent(SVC_SUNSHINE_TERMINATED, process_cstr);
        }
        break;
    }
