...
// macros
    ...
    #define TRAY_ICON "images/favicon.ico"
  #elif defined(__linux__) || defined(linux) || defined(__linux)
    #define TRAY_ICON "sunshine"
  #elif defined(__APPLE__) || defined(__MACH__)
    #define TRAY_ICON "images/logo-sunshine-16.png"
 ...
 */
  ...
  tray_open_ui_cb(struct tray_menu *item) {
    BOOST_LOG(info) << "Opening UI from system tray[Deactivated]"sv;
 ...
 */
  int
  system_tray() {
    //(Shawn) if dual display mode, change first menu
    if (config::video.haveSecondOutput) {
      tray.menu[0].text = "Open Sunshine (Dual Display Mode)";
    }
    if (tray_init(&tray) < 0) {
 ...
 */
  ...
  run_tray() {
    ...
    //Vector - per QA ticket GWS3-39, remove Sunshine icon from system tray
