...
namespace platf::dxgi {
    ...
    if (frame_update_flag) {
      {
        ...
        if (capture_format == DXGI_FORMAT_UNKNOWN) {
          ...
          BOOST_LOG(info) << "display_ram_t::snapshot Capture format ["sv << dxgi_format_to_string(capture_format) << ']';
  ...
  int
  display_ram_t::init(int displayId,const ::video::config_t &config, const std::string &display_name) {
