  ...
  float fontSize)
{
  ...
  const UINT textureWidth = texDesc.Width;
  const UINT textureHeight = texDesc.Height;
  ...
  // 5. 创建/重用 DirectWrite 工厂
  static ComPtr<IDWriteFactory> dwFactory;
  if (!dwFactory) {
      hr = DWriteCreateFactory(
          DWRITE_FACTORY_TYPE_SHARED,
          __uuidof(IDWriteFactory),          // 参数2: 接口的 GUID
    reinterpret_cast<IUnknown**>(dwFactory.GetAddressOf()) // 参数3: 指针地址
      );
      if (FAILED(hr)) return hr;
  }
  ...
  if (FAILED(hr)) return hr;
  // 9. 计算居中位置
  const float textWidth = textMetrics.width;
  const float textHeight = textMetrics.height;
  const D2D1_POINT_2F position = D2D1::Point2F(
    (textureWidth - textWidth) / 2.0f,
    (textureHeight - textHeight) / 2.0f
  );

  // 10. 创建旋转变换（绕文本中心旋转45度）
  const D2D1_POINT_2F rotationCenter = D2D1::Point2F(
    position.x + textWidth / 2.0f,
    position.y + textHeight / 2.0f
  );
  const D2D1_MATRIX_3X2_F rotateTransform = D2D1::Matrix3x2F::Rotation(
    -25.0f, // 旋转角度
    rotationCenter
  );
  // 11. 创建画笔
  ComPtr<ID2D1SolidColorBrush> textBrush;
  hr = d2dRenderTarget->CreateSolidColorBrush(textColor, &textBrush);
  if (FAILED(hr)) return hr;
  ...
  // 应用旋转矩阵
  d2dRenderTarget->SetTransform(rotateTransform);
  // 清空纹理为全透明
...
namespace platf::dxgi {
  ...
  public:
    ...
    convert(platf::img_t &img_base) override {
      ...
      device_ctx->PSSetShaderResources(1, 1, &watermarkSRV);
      device_ctx->Draw(3, 0);
    ...
    set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
      ...
      //auto scale_out_width = display->width * frame->height / display->height;
      //auto scale_out_height = display->height * frame->width / display->width;
    ...
    init(
      ...
      UINT creationFlags = D3D11_CREATE_DEVICE_FLAGS;
#ifndef NDEBUG
      ...
      float tileCount_in[16 / sizeof(float)] { tileCount };  //aligned to 16-byte
      tileCount_ps = make_buffer(device.get(), tileCount_in);
      if (!tileCount_ps) {
        BOOST_LOG(error) << "Failed to create tileCount_ps buffer"sv;
        return -1;
      }

      // 创建纹理描述
      D3D11_TEXTURE2D_DESC texDesc = {};
      texDesc.Width = this->display->width / tileCount;            // 水印宽度
      texDesc.Height = this->display->height / tileCount;           // 水印高度
      texDesc.MipLevels = 1;
      texDesc.ArraySize = 1;
      texDesc.Format = DXGI_FORMAT_B8G8R8A8_UNORM; // 必须支持RenderTarget
      texDesc.SampleDesc.Count = 1;
      texDesc.Usage = D3D11_USAGE_DEFAULT;
      texDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET;

      // 创建纹理
      HRESULT hr = device->CreateTexture2D(&texDesc, nullptr, &watermarkTexture);
      if (FAILED(hr)) {
        BOOST_LOG(error) << "Failed to create point sampler state [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
      srvDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // RGBA格式
      srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
      device->CreateShaderResourceView(watermarkTexture.get(), nullptr, &watermarkSRV);

      hr = DrawTextToTexture(
        device_ctx.get(),      // ID3D11DeviceContext*
        watermarkTexture.get(),   // 目标纹理
        string_to_wstring(config::stream.userName),    // 文本内容
        D2D1::ColorF(1.0f, 1.0f, 1.0f, 0.1f), // RGBA (Alpha=0.5)
        L"Segoe UI",        // 字体
        texDesc.Height * 0.1              // 字号
    );
    if (FAILED(hr)) {
      BOOST_LOG(error) << "Failed to create point sampler state [0x"sv << util::hex(status).to_string_view() << ']';
      return -1;
      ...
      device_ctx->PSSetConstantBuffers(0, 1, &color_matrix);
      device_ctx->PSSetConstantBuffers(1, 1, &tileCount_ps);
      device_ctx->VSSetConstantBuffers(0, 1, &info_scene);
  ...
  public:
    ...
    texture2d_t watermarkTexture;
    D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
    shader_res_t watermarkSRV;
    buf_t tileCount_ps;
    float tileCount = 4.0;
  ...
  display_vram_t::snapshot(platf::img_t *img_base, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    if (frame_info.LastMouseUpdateTime.QuadPart) {
      cursor_alpha.set_pos(frame_info.PointerPosition.Position.x, frame_info.PointerPosition.Position.y, 
        width_after_rotation, height_after_rotation, display_rotation, frame_info.PointerPosition.Visible);
    ...
    if (frame_update_flag) {
      ...
      if (capture_format == DXGI_FORMAT_UNKNOWN) {
        ...
        //BOOST_LOG(info) << "display_vram_t::snapshot Capture format ["sv << dxgi_format_to_string(capture_format) << ']';
  ...
  int
  display_vram_t::init(int displayId,const ::video::config_t &config, const std::string &display_name) {
    if (display_base_t::init(displayId,config, display_name)) {
      return -1;
    }
    ...
    status = device->CreateVertexShader(scene_vs_hlsl->GetBufferPointer(), 
      scene_vs_hlsl->GetBufferSize(), nullptr, &scene_vs);
    if (status) {
      BOOST_LOG(error) << "Failed to create scene vertex shader [0x"sv << util::hex(status).to_string_view() << ']';
      return -1;
    }
    ...
    if (config.dynamicRange && is_hdr()) {
      ...
      status = device->CreatePixelShader(scene_NW_ps_hlsl->GetBufferPointer(), 
        scene_NW_ps_hlsl->GetBufferSize(), nullptr, &scene_ps);
      if (status) {
      ...
      status = device->CreatePixelShader(scene_NW_ps_hlsl->GetBufferPointer(), 
        scene_NW_ps_hlsl->GetBufferSize(), nullptr, &cursor_ps);
      if (status) {
        BOOST_LOG(error) << "Failed to create cursor pixel shader [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
      ...
      status = device->CreatePixelShader(scene_ps_hlsl->GetBufferPointer(),
        scene_ps_hlsl->GetBufferSize(), nullptr, &scene_ps);
      if (status) {
        BOOST_LOG(error) << "Failed to create scene pixel shader [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
      ...
      status = device->CreatePixelShader(scene_ps_hlsl->GetBufferPointer(), 
        scene_ps_hlsl->GetBufferSize(), nullptr, &cursor_ps);
      if (status) {
        BOOST_LOG(error) << "Failed to create scene pixel shader [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
