...
namespace platf {
  ...
  abs_mouse(input_t &input, const touch_port_t &touch_port, float x, float y) {
    ...
    mi.dx = scaled_x;
    mi.dy = scaled_y;
    // BOOST_LOG(info) << "[xiaoqiang] mi.dx : " << mi.dx << ", mi.dy : " << mi.dy;
  ...
  SimulateCtrlV() {
      ...
      inputs[0].ki.wVk = VK_CONTROL;  // 虚拟键码：Ctrl

      // 按下 V 键
      inputs[1].type = INPUT_KEYBOARD;
      inputs[1].ki.wVk = 'V';          // 虚拟键码：V
  ...
  // 写入 UTF-8 到剪贴板
  bool paste(char* utf8Text, int utf8Size) {
    if (!OpenClipboard(nullptr)) return false;
    EmptyClipboard();

    int wlen = MultiByteToWideChar(CP_UTF8, 0, utf8Text, utf8Size, nullptr, 0);
    if (wlen == 0) {
      CloseClipboard();
      return false;
    }

    HGLOBAL hClip = GlobalAlloc(GMEM_MOVEABLE, (wlen + 1) * sizeof(wchar_t));
    if (!hClip) {
        CloseClipboard();
        return false;
    }
    wchar_t* pUnicode = static_cast<wchar_t*>(GlobalLock(hClip));
    if (!pUnicode) {
      GlobalFree(hClip); // GlobalLock 失败时必须释放！
      CloseClipboard();
      return false;
    }

    MultiByteToWideChar(CP_UTF8, 0, utf8Text, -1, pUnicode, wlen);
    GlobalUnlock(hClip);

    if (!SetClipboardData(CF_UNICODETEXT, hClip)) {
      GlobalFree(hClip); // SetClipboardData 失败时需要释放！
      CloseClipboard();
      return false;
    }
  ...
  void SendNewLine() {
    ...
    input[0].ki.wVk = VK_RETURN; // 回车键的虚拟键码
  ...
  unicode(input_t &input, char *utf8, int size) {
    ...
    // There is problem with the Enter key sending a line break
    // if (size == 1 && *utf8 == '\n') {
    //   SendNewLine();
    //   return;
    // }
