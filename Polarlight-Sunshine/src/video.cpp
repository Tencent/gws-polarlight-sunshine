...
#define ENCODE_RUN_FAILED_LOOP_CNT 100


using namespace std::literals;
namespace video {
  void
  get_output_names(std::string &output_Name, std::string &output2_Name) {
    output_Name = config::video.output_name;
    output2_Name = config::video.output2_name;
  }
  void
  set_output_names(int displayId, std::string &name) {
    if (displayId == 1) {
      config::video.output_name = name;
      SetWordbooks("config::video.output_name", (char *) config::video.output_name.c_str());
    }
    else if (displayId == 2) {
      config::video.output2_name = name;
      SetWordbooks("config::video.output2_name", (char *) config::video.output2_name.c_str());
    }
  }
  void
  clear_output2_name() {
  ...
  bool
  get_haveSecondOutput() {
    return config::video.haveSecondOutput;
  }
  std::vector<std::string>
  findCommonElements(const std::vector<std::string> &vec1, const std::vector<std::string> &vec2) {
  ...
  std::vector<std::string>
  findDifference(const std::vector<std::string> &vec1, const std::vector<std::string> &vec2) {
  ...
  static bool g_display2_changed = false;
  static bool g_user_flag = false;

  void
  StopVideoStreaming(int streamId) {
    auto video_stream_op_event = mail::man->event<int>(
    streamId == 2 ? mail ::video2_stream_op : mail ::video_stream_op);
    video_stream_op_event->raise(streamId);
  }
  void
  set_display_changed(std::vector<std::string> &output_list) {
    BOOST_LOG(info) << "in video::set_display_changed()";
    _display_changed_mutex.lock();
    if (output_list.size() > 1) {
        config::video.haveSecondOutput = true;
        if (config::video.output_name != output_list[0]) {
          g_display_changed = true;
          config::video.output_name = output_list[0];
      }
        if (config::video.output2_name != output_list[1]) {
          g_display2_changed = true;
          config::video.output2_name = output_list[1];
        }
    }
    else if (output_list.size() > 0) {
        //(Shawn)even just one display, we still keep second output 
        //for client fast turn on second display
        if (config::video.output_name != output_list[0]) {
          g_display_changed = true;
          config::video.output_name = output_list[0];
        }
        if(config::video.output2_name != DIPS_PLACEHOLDER) {
          config::video.output2_name = DIPS_PLACEHOLDER;
          config::video.haveSecondOutput = true;  //Keep true !!!
          StopVideoStreaming(2);
          //TODO: check if we need to send out g_display2_changed as true
          g_display2_changed = true;
        }
    }
    ...
    BOOST_LOG(info) << "in video::set_display_changed() config::video.haveSecondOutput = " 
      << config::video.haveSecondOutput;
    BOOST_LOG(info) << "in video::set_display_changed() output_name = " 
      << config::video.output_name;
    if (config::video.haveSecondOutput) {
      BOOST_LOG(info) << "in video::set_display_changed() output2_name = " 
        << config::video.output2_name;
    }
  }
  int
  getUserflag() {
  ...
  void
  setUserflag(bool flag) {
    _display_changed_mutex.lock();
    g_user_flag = flag;
    _display_changed_mutex.unlock();
  }
  bool
  IsDisplay_changed(int nDisplayID) {
    bool bChanged = false;
    _display_changed_mutex.lock();
    if (nDisplayID == 1) {
    ...
    else if (nDisplayID == 2) {
      bChanged = g_display2_changed;
      g_display2_changed = false;
    }
    _display_changed_mutex.unlock();
    return bChanged;
  }
  std::string
  GetOutputName(int displayIndex) {
    if (displayIndex == 1) {
...
#undef _CONVERT
  ...
  static void
  calc_fps(long long num_of_frames,
    double elapsed_time_ms, int displayIndex, bool isEncode = false) {

    double fps_sample = (double) (num_of_frames / elapsed_time_ms) * 1000.0;
    if (isEncode) {
      fps_mutex_encode.lock();
      g_fps_encode[displayIndex - 1] = fps_sample;
      // BOOST_LOG(info) << "*************g_fps_encode calc_fps: elapsed_time_ms: " << elapsed_time_ms << ",
      // index = "<< displayIndex <<", num_of_frames = " << num_of_frames << ",
      // fps = " << g_fps_encode[displayIndex-1] << std::endl;
      fps_mutex_encode.unlock();
    } else {
      fps_mutex.lock();
      g_fps[displayIndex - 1] = fps_sample;
      // BOOST_LOG(info) << "*************g_fps calc_fps: elapsed_time_ms: "
      // << elapsed_time_ms << ", index = "<< displayIndex <<", num_of_frames = "
      // << num_of_frames << ", fps = " << g_fps[displayIndex-1] << std::endl;
      fps_mutex.unlock();
    }
  }
  ...
  captureThread(
    ...
    int displayIndex, safe::mail_t mail) {
    BOOST_LOG(info) << "CaptureThread Started for Display:"sv << displayIndex;
    //videoInitThread.join();
    auto shutdown_event = mail->event<bool>(mail::shutdown); // mail from session
    ...
    auto fg = util::fail_guard([&]() {
      ...
      MetricsManager::I().reset_metrics();
    ...
    auto video_stream_op_event = mail::man->event<int>(
        displayIndex == 2?mail ::video2_stream_op:mail ::video_stream_op);
    ...
    if (displayIndex == 1) {
      MetricsManager::I().register_metrics(
        "FPS", 0, 1, [&]() {
        frames_mutex.lock();
        double fps = g_fps[0];
        frames_mutex.unlock();
    ...
    auto t_last_cal = t_start;
    auto f_last_cal = num_of_frames;
    decltype(t_start) t_tmp;
    ...
    while (capture_ctx_queue->running()) {
      ...
      if (displayIndex == 2 && config::video.haveSecondOutput == false) {
        BOOST_LOG(info) <<"CaptureThread: no second output, exit from loop"sv;
        break;
      }
      auto status = disp->capture([&](std::shared_ptr<platf::img_t> &img, bool frame_captured) -> std::shared_ptr<platf::img_t> {
        if (shutdown_event->peek())
          return nullptr;
        //BOOST_LOG(info) <<"Capture Callback:"<<frame_captured << "," << "displayIndex = " << displayIndex <<std::endl;
        num_of_frames++;
        ...
        while (capture_ctx != std::end(capture_ctxs)) {
          ...
          elapsed_time_ms = std::chrono::duration<double, std::milli>(t_tmp - t_last_cal).count();
          if (elapsed_time_ms > 1000) {
              // BOOST_LOG(info) << "*************Capture frame number: " << num_of_frames;
              calc_fps(num_of_frames - f_last_cal, elapsed_time_ms, displayIndex);
              f_last_cal = num_of_frames;
              t_last_cal = t_tmp;
          }
        ...
        //(Shawn)check if this stream need to be stopped
        if (video_stream_op_event->peek()) {
          BOOST_LOG(info) << "Received Video Stream Stop Event,StreamId = "sv << displayIndex;
          return nullptr;
        }
  ...
  make_session(platf::display_t *disp, const encoder_t &encoder, const config_t &config, int width, int height, std::shared_ptr<platf::hwdevice_t> &&hwdevice) {
    ...
    BOOST_LOG(info) << "make_session(), start ";
    ...
    ctx->width = (config.width == 0 ? width : width * config.height / height);
    ctx->height = (config.height == 0 ? height : config.height);
    ctx->time_base = AVRational { 1, config.framerate };
    ctx->framerate = AVRational { config.framerate, 1 };
    // BOOST_LOG(error) << "[xiaoqiang]" << ctx->width << " : " << ctx->height;
    ...
    if (hardware) {
      ...
      if (buf_or_error.has_right()) {
        BOOST_LOG(info) << "make_session(), pass 1";
        return std::nullopt;
      }
      hwdevice_ctx = std::move(buf_or_error.left());

      BOOST_LOG(info) << "make_session(), pass 2";
      ...
      if (encoder.derived_dev_type != AV_HWDEVICE_TYPE_NONE) {
        ...
        BOOST_LOG(info) << "make_session(), pass 3";
        ...
        BOOST_LOG(info) << "make_session(), pass 7";
      if (hwframe_ctx(ctx, hwdevice.get(), hwdevice_ctx, sw_fmt)) {
          BOOST_LOG(info) << "make_session(), pass 8";
      ...
      BOOST_LOG(info) << "make_session(), pass 9";
      ...
      ctx->slices = std::max(config.slicesPerFrame, config::video.min_threads);
        ...
        BOOST_LOG(info) << "make_session(), pass 10";
    ...
    BOOST_LOG(info) << "make_session(), pass 11";
    ...
    BOOST_LOG(info) << "make_session(), pass 12";
    ...
    BOOST_LOG(info) << "make_session(), pass 13";

    auto ctxGet = ctx.get();
    BOOST_LOG(info) << "make_session(), pass 13-1, ctxGet = " << ctxGet;

    if (auto status = avcodec_open2(ctxGet, codec, &options)) {
      BOOST_LOG(info) << "make_session(), pass 13-2";
    ...
    BOOST_LOG(info) << "make_session(), pass 13-3";

    frame_t frame { av_frame_alloc() };
    frame->format = ctx->pix_fmt;
    frame->width = ctx->width;
    frame->height = ctx->height;
    BOOST_LOG(info) << "make_session(), pass 14";
    ...
    BOOST_LOG(info) << "end of make_session()";
  ...
  encode_run(
    ...
    const encoder_t &encoder,
    void *channel_data) {
    BOOST_LOG(info) << "*************encode_run,call make_session,displayIndex="<<displayIndex << std::endl;
    stream::session::Inc_Unsafe_stop();
    auto session = make_session(disp.get(), encoder, config, disp->width, disp->height, std::move(hwdevice));
    stream::session::Dec_Unsafe_stop();
    if (!session) {
      BOOST_LOG(info) << "*****Can't Make Session********encode_run,displayIndex="<<displayIndex << std::endl;
      return -1001;
    }
    ...
    while (true) {
      ...
      if ((displayIndex == 2) && (config::video.haveSecondOutput == false)) {
        BOOST_LOG(info) << "*************exit from encode_run(), while loop, no second output but displayIndex =" 
          << displayIndex 
          << std::endl;
        break;
      }
      ...
      if (shutdown_event->peek() || reinit_event.peek() || !images->running()) {
        BOOST_LOG(info) << "*************encode_run: in while loop & break 1:displayIndex =" 
          << displayIndex 
          << ",shutdown_event->peek() = " 
          << shutdown_event->peek()
          << std::endl;
      ...
      elapsed_time_ms = std::chrono::duration<double, std::milli>(t_tmp - t_last_cal).count();
      if (elapsed_time_ms > 1000) {
        // BOOST_LOG(info) << "*************encode frame number: " << num_of_frames;
        calc_fps(num_of_frames - f_last_cal, elapsed_time_ms, displayIndex, true);
        f_last_cal = num_of_frames;
        t_last_cal = t_tmp;
      }
  ...
  make_synced_session(platf::display_t *disp, const encoder_t &encoder, platf::img_t &img, sync_session_ctx_t &ctx) {
    ...
    hdr_info_t hdr_info = std::make_unique<hdr_info_raw_t>(false, 1);
  ...
  encode_run_sync(
    ...
    // MetricsManager::I().register_metrics(
    //   "FPS_ENCODE", 0, 3, [&]() { return g_fps_encode[0]; },
    //   []() {
    //     g_fps_encode[0] = 0.0;
    //   });
    // MetricsManager::I().register_metrics(
    //   "FPS_ENCODE2", 0, 4, [&]() { return g_fps_encode[1]; },
    //   []() {
    //     g_fps_encode[1] = 0.0;
    //   });
  ...
  capture_async(
    ...
    auto lg = util::fail_guard([&]() {
      ...
      BOOST_LOG(info) << "*************capture_async::fail_guard displayIndex =" << displayIndex ;
    });
    // create a capture_thread_async_ctx_t,start capture thread
    auto ref = (displayIndex == 1) ? capture_thread_async.ref((char*)&mail) : capture_thread_async2.ref((char*)&mail);
    if (!ref) {
      return;
    }
    ...
    platf::adjust_thread_priority(platf::thread_priority_e::high);
    int iCntEncodeRunFailed = 0;
    while (true) {
      bool bNotShut = !shutdown_event->peek();
      bool bImg =  images->running();
      if (!bNotShut){ // if shutdown
        BOOST_LOG(info) << "*************capture_async(), shutdown_event->peek  " << displayIndex << std::endl;
        if(bImg){
          BOOST_LOG(info) << "============capture_async(), images->running  " << displayIndex << std::endl;
        }
      }
      ...
      if ((displayIndex == 2) && (config::video.haveSecondOutput == false)) {
        BOOST_LOG(info) << "*************capture_async(), while loop, no second output but displayIndex =" 
          << displayIndex 
          << std::endl;
      ...
      // Update client with our current HDR display state
      hdr_info_t hdr_info = std::make_unique<hdr_info_raw_t>(false, displayIndex);
      if (config.dynamicRange && display->is_hdr()) {
        display->get_hdr_metadata(hdr_info->metadata);
        hdr_info->enabled = true;
      }
      hdr_event->raise(std::move(hdr_info));
      BOOST_LOG(info) << "before encode_run:displayIndex =" << displayIndex << "  frame_nr = " << frame_nr << std::endl;
      int ret = encode_run(
        displayIndex,
        frame_nr,
        mail, images,
      ...
      if (ret != 0){
        if ( ++iCntEncodeRunFailed > ENCODE_RUN_FAILED_LOOP_CNT){
          BOOST_LOG(info) << "encode run failed failed break; code: " << ret << "  count: " 
          << iCntEncodeRunFailed << "  displayIndex = " << displayIndex << std::endl;
          break;
        }
        BOOST_LOG(info) << "encode run failed failed code: " << ret << "  count: " 
        << iCntEncodeRunFailed << "  displayIndex = " << displayIndex << std::endl;
        std::this_thread::sleep_for(100ms);
      }
      iCntEncodeRunFailed = 0;
      BOOST_LOG(info) << "after encode_run:displayIndex =" << displayIndex << "  frame_nr = " << frame_nr << std::endl;
    }

    BOOST_LOG(info) << "============exit from capture_async()  " << displayIndex << std::endl;
  }
  ...
  capture(
    ...
    //videoInitThread.join();
    BOOST_LOG(info) << "video::capture ... " << displayIndex;
    auto idr_events = (displayIndex == 1) ? mail->event<bool>(mail::idr) : mail->event<bool>(mail::idr2);
  ...
  validate_config(int displayIndex, std::string &displayName, std::shared_ptr<platf::display_t> &disp,
    ...
    bool bOK = reset_display(disp, encoder.base_dev_type, displayIndex, displayName, config);
    if ((!bOK) || (!disp)) {
      return -1;
    }

    auto pix_fmt = 
      config.dynamicRange == 0 ? map_pix_fmt(encoder.static_pix_fmt) : map_pix_fmt(encoder.dynamic_pix_fmt);
    auto hwdevice = disp->make_hwdevice(pix_fmt);
    if (!hwdevice) {
      return -1;
    }
    ...
    auto session = make_session(disp.get(), encoder, config, disp->width, disp->height, std::move(hwdevice));
    if (!session) {
      BOOST_LOG(error) << "In validate_config(), failed to make session for display index "sv << 
        displayIndex << ", displayName = "sv << displayName;
      return -1;
    }
  ...
  validate_encoder(encoder_t &encoder, std::string &displayName, int displayIndex) {
    ...
    int max_ref_frames_h264;
    int autoselect_h264;
    for(int i = 0; i < 10; ++i) // retry 
    {
      BOOST_LOG(info) << "validate_encoder validate_config h264 cnt " << i;
      max_ref_frames_h264 = validate_config(displayIndex, displayName, disp, encoder, config_max_ref_frames);
      autoselect_h264 = validate_config(displayIndex, displayName, disp, encoder, config_autoselect);
      ...
      if (max_ref_frames_h264 < 0 && autoselect_h264 < 0) {
        if (encoder.h264.qp && encoder.h264[encoder_t::CBR]) {
          ...
          BOOST_LOG(error) << "validate_encoder validate_config h264 failed, return";
        return false;
      }
      BOOST_LOG(info) << "validate_encoder validate_config h264 success";
      break;
    }
    ...
    if (test_hevc) {
      ...
      int max_ref_frames_hevc;
      int autoselect_hevc;
      for(int i = 0; i < 10; ++i) // retry 
      {
        BOOST_LOG(info) << "validate_encoder validate_config hevc cnt " << i;
        max_ref_frames_hevc = validate_config(displayIndex, displayName, disp, encoder, config_max_ref_frames);
        autoselect_hevc = validate_config(displayIndex, displayName, disp, encoder, config_autoselect);
      ...
      if (max_ref_frames_hevc < 0 && autoselect_hevc < 0) {
        ...
        if (force_hevc) {
            BOOST_LOG(error) << "validate_encoder validate_config hevc failed, return";
          return false;
        }
      }
        BOOST_LOG(info) << "validate_encoder validate_config hevc success";
        break;
      }
  ...
  initPerDisplay(std::vector<encoder_t> &encoders, std::string &defaultEncoder,
    ...
    BOOST_LOG(warning) << "initPerDisplay(), outputDisplayName = " << 
      outputDisplayName << ", Display Index = " << displayIndex;
...
#define TRIMOFF_CHARS " \t\n\r\f\v"
  ...
  template <typename T>
  bool
  compare(std::vector<T> &v1, std::vector<T> &v2) {
  ...
  int
  init_only() {
    if (config::video.output_name.size() == 0)
    {
      ...
      do {
        BOOST_LOG(info) << "try EnumDisplayDevices() for [" << retry << "]" << std::endl;
        while (EnumDisplayDevices(nullptr, displayIndex, &displayDevice, 0)) {
          if(displayDevice.StateFlags & DISPLAY_DEVICE_ACTIVE) {
            std::string outName(displayDevice.DeviceName);
            if (displayDevice.StateFlags & DISPLAY_DEVICE_REMOTE || 
                std::string(displayDevice.DeviceString).find("Basic") != std::string::npos )
            {
              BOOST_LOG(info) << "init_only(), EnumDisplayDevices ignore Remote or Basic: " << outName << std::endl;
            }
            else{
              output_list.push_back(outName);
              BOOST_LOG(info) << "init_only(), EnumDisplayDevices:" 
                << outName 
                << ", StateFlags: " 
      ...
      } while (++retry < max_retry && output_list.size() == 0);

      if (output_list.size() > 0)
      {
        config::video.output_name = output_list[0];
        BOOST_LOG(info) << "init_only(), EnumDisplayDevices, set output_name = " << 
          config::video.output_name << std::endl;
      }
      else
      {
        BOOST_LOG(info) << "init_only(), no displays" << std::endl;
      }
    }
    BOOST_LOG(info) << "init encoders for first display";
    SetWordbooks("config::video.output_name", (char *) config::video.output_name.c_str());
    SetWordbooks("config::video.output2_name", (char *) config::video.output2_name.c_str());
    CopyEncoder(encoders_backup, encoders);
    int retValue = initPerDisplay(encoders, config::video.encoder, config::video.output_name, 1);
    if (retValue != 0) {
        BOOST_LOG(info) << "init_only(), initPerDisplay failed, output_name = " << 
          config::video.output_name << ", encoder =" << config::video.encoder;
        return retValue;
    }
    BOOST_LOG(info) << "init_only(), initPerDisplay successful, output_name = " << 
      config::video.output_name << ", encoder =" << config::video.encoder;
    BOOST_LOG(info) << "init_only(), config::video.hevc_mode: "sv << config::video.hevc_mode;
    //Second encoders2 just copy from first group
    CopyEncoder(encoders, encoders2);
    return 0;
  }
  ...
  bool
  ValidateDisplay(const char *display) {
    BOOST_LOG(info) << "ValidateDisplay(), at the beginning";
    std::vector<encoder_t> v_encoders;
    CopyEncoder(encoders_backup, v_encoders);
    std::string displayName = display;
    std::string defaultEncoder= config::video.encoder;
    int retValue = initPerDisplay(v_encoders, defaultEncoder, displayName, -1);
    if (retValue != 0) {
      BOOST_LOG(info) << "ValidateDisplay(), initPerDisplay failed, output_name = " << 
        displayName << ", encoder =" << defaultEncoder;
      return false;
    }
    else {
      BOOST_LOG(info) << "ValidateDisplay(), initPerDisplay successful, output_name = " << 
        displayName << ", encoder =" << defaultEncoder;
      return true;
    }
  }
...
namespace video {
  ...
  start_capture_async(capture_thread_async_ctx_t &capture_thread_ctx, char *data) {
    ...
    safe::mail_t mail = *(safe::mail_t*)data;
