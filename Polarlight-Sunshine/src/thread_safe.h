...
namespace safe {
  ...
  public:
    ...
    //(Shawn)The pop method now uses wait_for with a timeout of 100 milliseconds. 
    //This ensures the method periodically wakes up to check the termination predicate.
    status_t
    pop(std::function<bool()> terminate_pred) {
      std::unique_lock ul { _lock };
      ...
      while (_queue.empty()) {
        if (_cv.wait_for(ul, std::chrono::milliseconds(100), [this]() { 
          ...
          // Check the terminate predicate periodically
          if (terminate_pred()) {
            return util::false_v<status_t>;
          }
        }
  ...
  public:
    ...
    using construct_f = std::function<int(element_type&, char* buf)>;
    ...
    [[nodiscard]] ptr_t ref(char *data = NULL) 
    {
        std::lock_guard lg { _lock };
        ...
        if (!_count) 
        {
            ...
            if (_construct(*reinterpret_cast<element_type *>(_object_buf.data()), data)) 
            {
