...
#pragma pack(pop)
  ...
  // return -1 on error
  static inline int
  encode_audio(int featureSet, const audio::buffer_t &plaintext, audio_packet_t &destination,
    std::uint32_t avRiKeyIv, crypto::cipher::cbc_t &cbc, char *AesKey) {
    ...
    *(std::uint32_t *) iv.data() = util::endian::big<std::uint32_t>(avRiKeyIv + destination->rtp.sequenceNumber);
    ...
    return cbc.encrypt(
      std::string_view { (char *) std::begin(plaintext), plaintext.size() }, destination->payload(), &iv, AesKey);
  }


std::string GetSystemEventLogPath() {
    char buffer[MAX_PATH] = {};
    GetModuleFileNameA(NULL, buffer, MAX_PATH);
    std::string exePath(buffer);
    std::string exeDirectory = exePath.substr(0, exePath.find_last_of("\\/"));
    return exeDirectory + "\\log\\system_event.log";
}
...
std::string GetCurrentTimeAndZone() {
    std::time_t now_time = GetCurrentTimeStamp();
    std::tm local_tm;
    ...
    oss << std::setw(2) << std::setfill('0') << bias / 60 << ":"
        << std::setw(2) << std::setfill('0') << bias % 60 << ")";
    return oss.str();
}

void WriteReportEventToLog(int id, const char *pStr) {
    if (!pStr)
      return;
    std::string systemLogPath = GetSystemEventLogPath();
    FILE * fp = NULL;
    fopen_s(&fp, systemLogPath.c_str(), "a+");
    if (!fp)
      return;
    uint64_t ts = static_cast<uint64_t>(GetCurrentTimeStamp());
    fprintf(fp, "%s, time_stamp = %llu, id = %d, details = %s\n", GetCurrentTimeAndZone().c_str(), ts, id, pStr);
    fclose(fp);
}
...
#define SVC_CONNECT ((DWORD) 50001)
  void
  WriteReportEvent(int dwID, const char *pStr) {
    ...
    WriteReportEventToLog(dwID, pStr);
    //delete[]wide;
#endif
  }
  static inline void
 ...
 */
  ...
  int
  start_broadcast(broadcast_ctx_t &ctx, char *data);
  void
    ...
    if (res > 0) {
      ...
      switch (event.type) {
        ...
        case ENET_EVENT_TYPE_DISCONNECT:
          ...
          BOOST_LOG(info) << "CLIENT DISCONNECTED client uuid = "sv << session->szClientUUid;
          OnClientStatusChanged(session->szClientUUid.c_str(), SunshineStatus::ClientStatus_Disconnect);
          MetricsManager::I().reset_metrics();
  ...
  send_hdr_mode(session_t *session, video::hdr_info_t hdr_info) {
    ...
    plaintext.enabled |= (0x80 >> (hdr_info->displayIndex - 1));
  ...
  controlBroadcastThread(control_server_t *server) {
    ...
    auto metrics_init = [](const char *name, const int streamId, const int index) {
      ...
      MetricsManager::I().register_metrics(
        name, 1, offset + 1, [streamId, index, offset]() {
          int idx = (streamId - 1) * cnt_per_stream + index;         
          metrics_lock.lock();
          float val = metrics[idx];
          metrics_lock.unlock();
          return val; },  //function to get the counter
    ...
    BOOST_LOG(info) << "Exit from controlBroadcastThread"sv;
  }
  ...
  recvThread(broadcast_ctx_t &ctx) {
    ...
    auto recv_func_init = [&](udp::socket &sock, int buf_elem) {
      recv_func[buf_elem] = [&, buf_elem](const boost::system::error_code &ec, size_t bytes) {
        ...
        BOOST_LOG(verbose) << "Recv: "sv << peer[buf_elem].address().to_string()
                           << ':' << peer[buf_elem].port() << " :: " << type_str;
        ...
        if (it != std::end(peer_to_session[buf_elem])) {
          ...
          peer_lock.unlock();
          BOOST_LOG(debug) << "RAISE: "sv << peer[buf_elem].address().to_string()
                           << ':' << peer[buf_elem].port() << " :: " << type_str;
        }
        else {
          peer_lock.unlock();
  ...
  void
  videoBroadcastThread(udp::socket &sock, int streamId, char *AESKey) {
    auto shutdown_event = mail::man->event<bool>(mail::broadcast_shutdown);
    ...
    while (auto packet = packets->pop()) {
      ...
      if( session == nullptr) {//TODO: find why here may be null
        break;
      }
    ...
    BOOST_LOG(info) << "Exit from videoBroadcastThread streamId: "sv << streamId;
  }
  ...
  audioBroadcastThread(udp::socket &sock, char *AESKey) {
    ...
    while (auto packet = packets->pop()) {
      ...
      if (AESKey) {
        ...
        bytes = encode_audio(session->config.featureFlags,
          packet_data, audio_packet, session->audio.avRiKeyId, session->audio.cipher, AESKey);
      }
      else {
        //"audio config  encrypt";
        bytes = encode_audio(session->config.featureFlags,
          packet_data, audio_packet, session->audio.avRiKeyId, session->audio.cipher, NULL);
      }
    ...
    shutdown_event->raise(true);
    BOOST_LOG(info) << "Exit from audioBroadcastThread"sv;
  }
  ...
  start_broadcast(broadcast_ctx_t &ctx, char *data) {
    ...
    BOOST_LOG(info) << "*************** begin config::video.haveSecondOutput" << config::video.haveSecondOutput;
    BOOST_LOG(info) << "*************** config::video.output_name = " << config::video.output_name;
    BOOST_LOG(info) << "*************** config::video.output2_name = " << config::video.output2_name;
    ...
    config::video.bChangeSecondOutputFlag = false;
    BOOST_LOG(info) << "*************** end config::video.afterhaveSecondOutput = " << config::video.afterhaveSecondOutput;
    BOOST_LOG(info) << "*************** end config::video.haveSecondOutput = " << config::video.haveSecondOutput;
    /*
    ...
    if (ctx.control_server.bind(control_port)) {
      BOOST_LOG(error) << "in session::start_broadcast(), Couldn't bind Control server to port ["sv << 
        control_port << "], likely another process already bound to the port"sv;
    ...
    if (ec) {
      BOOST_LOG(fatal) << "in session::start_broadcast(),Couldn't bind Video server to port ["sv << 
        video_port << "]: "sv << ec.message();
    ...
    if (config::video.haveSecondOutput) {
      ...
      if (ec) {
        BOOST_LOG(fatal) << "in session::start_broadcast(),Couldn't bind Video2 server to port ["sv << 
          video_port2 << "]: "sv << ec.message();
    ...
    if (ec) {
      BOOST_LOG(fatal) << "in session::start_broadcast(),Couldn't bind Audio server to port ["sv << 
        audio_port << "]: "sv << ec.message();
    ...
    video::setUserflag(true);
    return 0;
  }
  extern int g_user_flag;
  void
  end_broadcast(broadcast_ctx_t &ctx) {
    /* if (config::sunshine.privacy_mode) {
    ...
    video::setUserflag(false);

    BOOST_LOG(info) << "end_broadcast Userflag= " << video::getUserflag();
    auto broadcast_shutdown_event = mail::man->event<bool>(mail::broadcast_shutdown);
    ...
    video_packets->stop();
    if (config::video.afterhaveSecondOutput) {
      BOOST_LOG(info) << "video_packets2->stop()";
      if (video_packets2) {
        video_packets2->stop();
      }
    }
    ...
    if (config::video.afterhaveSecondOutput) {
      BOOST_LOG(info) << " ctx.video_sock2.close()";
    ...
    video_packets.reset();
    if (config::video.afterhaveSecondOutput) {
      BOOST_LOG(info) << " video_packets2.reset()";
      if (video_packets2) {
    ...
    // add event write here in case it is not written elsewhere
    WriteReportEvent(SVC_DISCONNECT, proc::proc.getCSession().c_str());
    BOOST_LOG(info);
  ...
  recv_ping(decltype(broadcast)::ptr_t ref, socket_e type, udp::endpoint &peer, std::chrono::milliseconds timeout) {
    ...
    while (current_time - start_time < config::stream.ping_timeout) {
      ...
      if (msg == ping) {
        BOOST_LOG(debug) << "Received ping from "sv 
          << peer.address() 
          << ':' 
          << port 
          << " ["sv 
          << util::hex_vec(msg) 
          << ']';
        std::cout << "Received ping from "sv 
          << peer.address() 
          << ':' 
          << port 
          << " ["sv 
      ...
      BOOST_LOG(debug) << "Received non-ping from "sv << peer.address()
                       << ':' << port << " ["sv << util::hex_vec(msg) << ']';
      std::cout << "Received non-ping from "sv << peer.address()
                << ':' << port << " ["sv << util::hex_vec(msg) << ']' << std::endl;

      current_time = std::chrono::steady_clock::now();
    }

    BOOST_LOG(error) << "Initial Ping Timeout"sv; // not use
    return -1;
  }
  uint16_t g_second_video_last_client_port = 0;
  //(Shawn)The new recv_ping function calls the pop method with the 
  // termination predicate, ensuring it can handle termination signals.
  int
  recv_ping(decltype(broadcast)::ptr_t ref, socket_e type, udp::endpoint &peer, 
      std::chrono::milliseconds timeout, std::function<bool()> shutdown_check) {
    auto constexpr ping = "PING"sv;
    ...
    while (current_time - start_time < config::stream.ping_timeout) {
      ...
      auto msg_opt = messages->pop(shutdown_check);
      if (!msg_opt) {
        break;
      }
      ...
      TUPLE_2D_REF(port, msg, *msg_opt);
      if (msg == ping) {
        if (type == socket_e::video2 && port == g_second_video_last_client_port)
        {
            if (--iCheckCnt > 0)
            {
                continue;
        ...
        BOOST_LOG(debug) << "Received ping from "sv << peer.address() << ':' << 
          port << " ["sv << util::hex_vec(msg) << ']';
        std::cout << "Received ping from "sv << peer.address() << ':' << port << 
          " ["sv << util::hex_vec(msg) << ']' << std::endl;
        ...
        if (type == socket_e::video2)
        {
          g_second_video_last_client_port = 0;
        }
        return port;
  ...
  //(Shawn) add this func to clear client Port and make next recv_ping
  //used to video stream stop when client window closed
  void
  ClearClientPortForPing(decltype(broadcast)::ptr_t ref,udp::endpoint &peer) {
    auto &connections = ref->audio_video_connections;
    auto lg = connections.lock();
    auto addr_str = peer.address().to_string();
    auto port = peer.port();
    for (auto it = std::begin(*connections); it != std::end(*connections); ++it) {
      TUPLE_2D_REF(addr, port_ref, *it);
      if (port == port_ref && addr_str == addr) {
        g_second_video_last_client_port = it->second;
        it->second = 0;//Clear
        break;
      }
    }
  ...
  videoThread(session_t *session, int displayIndex) {
    ...
    BOOST_LOG(info) << "videoThread() displayIndex = " << displayIndex;
    ...
    int loopCount = 0;
    // Define the shutdown check callback
    auto shutdown_check = [shutdown_event,session]() {
      return shutdown_event->peek() || (session->state != session::state_e::RUNNING);
    };
    while (session->state == session::state_e::RUNNING) {
      BOOST_LOG(info) << "Enter recv_ping, displayIndex = "sv << displayIndex <<",loopCount = "sv <<loopCount;
      auto port = recv_ping(ref, (displayIndex == 1) ? socket_e::video : socket_e::video2,
        video.peer, config::stream.ping_timeout, shutdown_check);
      BOOST_LOG(info) << "Exit recv_ping, displayIndex = "sv << displayIndex <<
        ",loopCount = "sv <<loopCount <<",port:"sv <<port;
      if(shutdown_check()) {
        break;
      }
      if (port < 0) {
        BOOST_LOG(info) << "Port <0,videoThread:displayIndex=" << displayIndex;
        return;
      }
      ...
      //but for second display, we need to wait to output2_name changed to non-placeholder
      if (displayIndex == 2) {
        while (config::video.output2_name == DIPS_PLACEHOLDER) {
          if (shutdown_check()) {
            break;
          }
          std::this_thread::sleep_for(1ms);
      ...
      BOOST_LOG(info) << "Start capturing Video,loopCount="sv << loopCount;
      video::capture(
        session->mail, displayIndex == 1 ? session->config.monitor : session->config.monitor2,
        session,
        displayIndex);
      loopCount++;
      BOOST_LOG(info) << "Exit from capturing Video,loopCount = "sv << loopCount;
      if(shutdown_event->peek()) {
          BOOST_LOG(info) << "Recv  shutdown_event, then exit"sv;
          break;
      }
      ClearClientPortForPing(ref,video.peer);
      // very important to reset lowseq to zero 2024.09.17 wangtong
      auto &sessionVideo = (displayIndex == 1) ? session->video : session->video2;
      sessionVideo.lowseq = 0;
    }
    BOOST_LOG(info) << "Exit from videoThread() displayIndex = "sv << displayIndex <<",loopCount = "sv <<loopCount;
  }
  void
  audioThread(session_t *session) {
    ...
    BOOST_LOG(info) << "Exit from audioThread"sv;
  }
  ...
  namespace session {
    ...
    join(session_t &session) {
      ...
      BOOST_LOG(debug) << "Removing references to any connections..."sv;
      {
        ...
        int diff = config::video.afterhaveSecondOutput ? 3 : 2;  //for dual videos support
    ...
    start(session_t &session, const std::string &addr_string) {
...

      {
        ...
        if (config::video.haveSecondOutput) {
          BOOST_LOG(info) << " connections->emplace_back config::video.haveSecondOutput = " << config::video.haveSecondOutput;
          connections->emplace_back(addr_string, 0);
        }
        connections->emplace_back(addr_string, 0);
      }
      ...
      if (config::video.haveSecondOutput) {
        BOOST_LOG(info) << " session.videoThread2";
    ...
    alloc(config_t &config, crypto::aes_t &gcm_key, crypto::aes_t &iv, char *AesKey, char *clientUUid) {
      ...
      if (clientUUid) {
        session->szClientUUid = clientUUid;
      }
      ...
      //OnSessionStatusChanged(100, clientUUid);
      return session;
    }
  }  // namespace session
}  // namespace stream
