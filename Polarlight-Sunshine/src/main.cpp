...
main(int argc, char *argv[]) {
  ...
  if (!config::sunshine.cmd.name.empty()) {
    ...
    (void)fn->second(argv[0], config::sunshine.cmd.argc, config::sunshine.cmd.argv);
    std::terminate();
  }
  BOOST_LOG(info) << std::endl 
    << "====================== " 
    << PROJECT_NAME 
    << " start ====================="; // = Sunshine start =
  BOOST_LOG(info) << PROJECT_NAME << " version: " << PROJECT_VER << std::endl; // Sunshine version:
  task_pool.start(1);
...
void InitLogging() {
  ...
  std::string rotate_log_file = config::sunshine.log_file;
  rotate_log_file = rotate_log_file.substr(0, rotate_log_file.size() - 4);
  rotate_log_file += "_%Y%m%d.log",
  log_file_sink = bl::add_file_log(bl::keywords::file_name = config::sunshine.log_file,
  ...
  console_sink->set_filter(severity >= config::sunshine.min_log_level);
  log_file_sink->set_filter(severity >= config::sunshine.min_log_level);

  auto formatter = [
      message = "Message"s,
      severity = "Severity"s
    ](const bl::record_view &view, bl::formatting_ostream &os) {
