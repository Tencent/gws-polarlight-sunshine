...
namespace rtsp_stream {
  ...
  std::atomic_bool isIniting = true;
  class rtsp_server_t;
  ...
  public:
    rtsp_server_t(){
      MetricsManager::I().register_metrics("Slots", 0, 5, [&]() {
      return _slot_count;
    }, 
    [](){
    }
    ...
    clear(bool all = true) {
      // if(all) {
      //   BOOST_LOG(info) << "rtsp->clear: all"sv;
      // }
      ...
      for (auto &slot : *_session_slots) {
        if (slot && (all || stream::session::state(*slot) == stream::session::state_e::STOPPING)) {
          ...
          ++_slot_count;
          //BOOST_LOG(info) << "rtsp->clear:loop into slots,count="sv<<_slot_count;
        }
      }
  ...
  send(tcp::socket &sock, const std::string_view &sv) {
    ...
    while (bytes_send != sv.size()) {
      ...
      if (ec) {
        ...
        server.clear();//clear all sessions
        BOOST_LOG(info) << "All Sessions cleared out"sv;
        return -1;
      }
    }
  ...
  cmd_setup(rtsp_server_t *server, tcp::socket &sock, msg_t &&req) {
    ...
    else if (type == "video2"sv) {
      port = map_port(stream::VIDEO_STREAM_PORT2);
    }
  ...
  cmd_announce(rtsp_server_t *server, tcp::socket &sock, msg_t &&req) {
    ...
    try {
      ...
      config.monitor2.height = config.monitor.height = util::from_view(args.at("x-nv-video[0].clientViewportHt"sv));
      config.monitor2.width = config.monitor.width = util::from_view(args.at("x-nv-video[0].clientViewportWd"sv));
      config.monitor2.framerate = config.monitor.framerate = util::from_view(args.at("x-nv-video[0].maxFPS"sv));
      if (args.find("x-nv-video[1].clientViewportHt"sv) != args.end()) {
        config.monitor2.height = util::from_view(args.at("x-nv-video[1].clientViewportHt"sv));
        config.monitor2.width = util::from_view(args.at("x-nv-video[1].clientViewportWd"sv));
        config.monitor2.framerate = util::from_view(args.at("x-nv-video[1].maxFPS"sv));
      }

      config.monitor2.bitrate = config.monitor.bitrate =
        util::from_view(args.at("x-nv-vqos[0].bw.maximumBitrateKbps"sv));
      config.monitor2.slicesPerFrame = config.monitor.slicesPerFrame =
        util::from_view(args.at("x-nv-video[0].videoEncoderSlicesPerFrame"sv));
      config.monitor2.numRefFrames = config.monitor.numRefFrames =
        util::from_view(args.at("x-nv-video[0].maxNumReferenceFrames"sv));
      config.monitor2.encoderCscMode = config.monitor.encoderCscMode =
        util::from_view(args.at("x-nv-video[0].encoderCscMode"sv));
      config.monitor2.videoFormat = config.monitor.videoFormat =
        util::from_view(args.at("x-nv-vqos[0].bitStreamFormat"sv));
      config.monitor2.dynamicRange = config.monitor.dynamicRange =
        util::from_view(args.at("x-nv-video[0].dynamicRangeMode"sv));
    }
    ...
    auto session = stream::session::alloc(
      config,
      launch_session->gcm_key,
      launch_session->iv,
      (char*)launch_session->AesKey.data(),
      (char *)launch_session->szClientUUid.c_str());
  ...
  rtpThread() {
    ...
    while (!shutdown_event->peek()) {
      ...
      if (broadcast_shutdown_event->peek()) {
        BOOST_LOG(fatal) << "**********rtpThread(), broadcast_shutdown_event peek()"sv; 
        server.clear();
      }
      else {
    ...
    BOOST_LOG(fatal) << "**********rtpThread(), shutdown_event"sv; 
