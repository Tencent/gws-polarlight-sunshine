...
//public key encrypt
bool RsaPubEncrypt(const char* clear_text, int size, const std::string& pub_key, std::vector<unsigned char>& outDataStr)
{
...
namespace crypto {
  ...
  openssl_verify_cb(int ok, X509_STORE_CTX *ctx) {
    ...
    switch (err_code) {
      ...
      // FIXME: Checking for X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT_LOCALLY is a
      // temporary workaround to get moonlight-embedded to work on the raspberry pi
  ...
  namespace cipher {
    ...
    gcm_t::encrypt(const std::string_view &plaintext, std::uint8_t *tagged_cipher, aes_t *iv) {
      ...
      if (EVP_EncryptUpdate(
        ...
        cipher,
        &size,
        (const std::uint8_t *)plaintext.data(),
        plaintext.size()) != 1) {
        return -1;
      }
    ...
    int
    cbc_t::encrypt(const std::string_view &plaintext, std::uint8_t *cipher, aes_t *iv, char* AesKey) {
        if (AesKey)
        {
            aes_t temp;
            memcpy(temp.data(), AesKey, 16);
            if (!encrypt_ctx && init_encrypt_cbc(encrypt_ctx, &temp, iv, padding)) {
                return -1;
            }
        }
        else
